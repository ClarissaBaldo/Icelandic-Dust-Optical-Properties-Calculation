...
rm_data <- list()
for (i in 1:length(Bsca_calculated)) {
  if ((colnames(Bsca_calculated[[i]])[3] %in% rm_WL) == F){
    rm_data[[i]] <- Bsca_calculated[[i]]}
  else {
    rm_data[[i]] <- NULL
  }
}
...
Bsca_calculated <- rm_data[lengths(rm_data) != 0]
...
split_data_byXnk <- function(df) {
  ...
  # create a list with the calculated Bsca for each scenario
  WL_scenario <- list()
...
Bsca_calculated_scenarios <- list()

for (i in 1:length(Bsca_calculated)) {
  Bsca_calculated_scenarios[[i]] <- split_data_byXnk(Bsca_calculated[[i]])
}

#############################################################################################################
# import the measured Bsca
#############################################################################################################

# import data
Bsca_data <- read.csv("Z:/Clarissa/Data_Optical_calculation/R_Code/Code_final/ICEdust_samples/Maeli2/Nep_Aet_Mie/original/data/Maeli2_Bsca_fitted_12min_original.csv")

Bsca_data$date  <- as.POSIXct(Bsca_data$date ,  format =  "%Y-%m-%d %H:%M:%S") # transform as POSIXct
...
# get mean Bsca measured values only
Bsca <- Bsca_data[,1:8]

# get error Bsca measured values only
Bsca_error <- Bsca_data[,-c(2:8, 16:22)]

# give column names
Aet_WL <- c(370, 470, 520, 590, 660, 880, 950)

for (i in 2:length(Bsca)) {
  colnames(Bsca)[i] <- sprintf("Bsca_meas_%d", Aet_WL[i-1])
}
...
Bsca$date  <- as.POSIXct(Bsca$date ,  format =  "%Y-%m-%d %H:%M:%S") # transform as POSIXct

for (i in 1:length(Bsca_calculated_scenarios)) {
  for (k in 1:length(Bsca_calculated_scenarios[[i]])) {
    
    Bsca_calculated_scenarios[[i]][[k]]$date <- as.POSIXct(Bsca_calculated_scenarios[[i]][[k]]$date ,  format =  "%Y-%m-%d %H:%M:%S") # transform as POSIXct
    
  }
}

# join Bsca calculated and measured dfs
Bsca_comparison <- Bsca_calculated_scenarios

for (i in 1:length(Bsca_calculated_scenarios)) {
  for (k in 1:length(Bsca_calculated_scenarios[[i]])) {
    
    Bsca_comparison[[i]][[k]] <- full_join(Bsca_calculated_scenarios[[i]][[k]], Bsca[,c(1,i+1)])
...
for (i in 1:length(Bsca_comparison)) {
  for (k in 1:length(Bsca_comparison[[i]])) {
    Bsca_comparison[[i]][[k]] <- add_Xnk(Bsca_comparison[[i]][[k]])
  }
}
...
for (i in 1:length(Bsca_comparison)) {
  for (k in 1:length(Bsca_comparison[[i]])) {
    Bsca_comparison[[i]][[k]]$X <- as.numeric(as.character(Bsca_comparison[[i]][[k]]$X))
    Bsca_comparison[[i]][[k]]$n <- as.numeric(as.character(Bsca_comparison[[i]][[k]]$n))
    Bsca_comparison[[i]][[k]]$k <- as.numeric(as.character(Bsca_comparison[[i]][[k]]$k))
  }
}
...
for (i in 1:length(Bsca_comparison)) {
  for (k in 1:length(Bsca_comparison[[i]])) {
    colnames(Bsca_comparison[[i]][[k]])[3] <- "model"
    colnames(Bsca_comparison[[i]][[k]])[4] <- "obs"
  }
}
...
for (i in 1:length(Bsca_comparison)){
  for (k in 1:length(Bsca_comparison[[i]])){
    if (Bsca_comparison[[i]][[k]]$X[1] != 1){
      rm_X1[[i]][[k]] <- Bsca_comparison[[i]][[k]]
    }
  }
}
...
Bsca_OG_results <- stat_comparison 

#############################################################################################################
# Repeat the calculations after subtracting the error to the measured Bsca
#############################################################################################################
...
rm(list=setdiff(ls(), c("Bsca_OG_results", "Bsca_pSD1_results","Bsca_mSD1_results",
                        ...
                        "Bsca_calculated_scenarios", "Bsca_data")))
...
# subtract the error to Bsca
for (i in 2:length(Bsca)) {
  Bsca[,i] <- Bsca[,i] - Bsca_error[,i]
}
...
#############################################################################################################
# Repeat the calculations after adding the error to the measured Bsca
#############################################################################################################
...
Bsca_pSD1_results <- stat_comparison 
